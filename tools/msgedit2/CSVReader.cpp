...
ColReader::ColReader(istream& in)
	: m_cin(in)
{
	m_pLogger = Logger::getLogger("CSVREADER");
}
...
ColReader::read(string& col)
{
	...
	m_pLogger->detail(string("ColReader::read(): returning col=") + col);
	return c;
}
...
ColReader::readQuotedCol(string& quotedCol) 
{
	...
	m_pLogger->detail(string("ColReader::readQuotedCol(): returning quotedCol=") + quotedCol);
	return c;
}
