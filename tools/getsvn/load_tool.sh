...
# Download source file from svn http site.
function _g_get_file() {
    local _file_path="${ENV_SVN_HOSTNAME_PORT}/viewvc/esa/branches/${1}/${2}?view=co"

    if [ x"$ENV_SVN_HOSTNAME_PORT" = x ]; then
        echo "You must define ENV_SVN_HOSTNAME_PORT !"
	return
    fi
...
# Launch web browser at source file from svn http site.
function _gb_get_file() {
    local _file_path="${ENV_SVN_HOSTNAME_PORT}/viewvc/esa/branches/${1}/${2}"
...
function _g_determine_branch() {
    ...
    case "$_str" in
    *V66*)  _branch='V66_Fixes_new'	;;
    *V711*) _branch='V711_oneoffs'	;;
